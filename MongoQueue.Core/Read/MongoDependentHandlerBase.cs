...
namespace MongoQueue.Core.Read
{
    public abstract class MongoDependentHandlerBase<TMessage> : IHandler
    {
        ...
        public async Task Handle(string appName, string messageId, object message, bool resend,
        ...
        private async Task<bool> SetProcessingStart(string appName, string messageId,
            CancellationToken cancellationToken)
        {
            var collection = _mongoMessagingAgent.GetEnvelops(appName);
            var filter = Builder.Filter.And(Builder.Filter.Eq("_id", messageId),
                Builder.Filter.Eq(x => x.ProcessedAt, DateTime.MinValue));
            var found = await collection.FindOneAndUpdateAsync<Envelope>(filter,
                Builder.Update.Set(x => x.ProcessingStartedAt, DateTime.UtcNow).Set(x => x.IsProcessingStarted, true),
                cancellationToken: cancellationToken);
            return found != null;
        }
        ...
        private async Task SetProcessedAt(string appName, string messageId, CancellationToken cancellationToken)
        {
            ...
            await collection.FindOneAndUpdateAsync<Envelope>(Builder.Filter.Eq("_id", messageId),
                Builder.Update.Set(x => x.ProcessedAt, DateTime.UtcNow).Set(x => x.IsProcessed, true),
                cancellationToken: cancellationToken);
        }
    }
}